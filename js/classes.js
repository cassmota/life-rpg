...
function openClassDetail(id){
  ...
  showMo(null, null,
    `<div style="text-align:center;margin-bottom:14px">
      <div style="display:flex;align-items:center;justify-content:center;margin-bottom:6px">${CLASS_SVG[id]?CLASS_SVG[id].replace('width="56" height="56"','width="72" height="72"'):('<span style="font-size:44px">'+cls.icon+'</span>')}</div>
...
function renderClasse(){
      ...
      cur.innerHTML = `
        <div style="background:linear-gradient(135deg,rgba(0,0,0,.5),rgba(0,0,0,.3));border:1px solid ${evolved.color}40;border-radius:8px;padding:12px">
          <div style="display:flex;align-items:center;gap:10px">
            <span style="display:inline-flex;align-items:center;justify-content:center;width:48px;height:48px">${CLASS_SVG[S.playerClass]||'<span style="font-size:32px">'+evolved.icon+'</span>'}</span>
            <div style="flex:1">
  ...
  grid.innerHTML = Object.values(CLASSES).map(cls => {
    ...
    return `<div class="class-card cls-${cls.id} ${isActive?'selected':''} ${(isLocked||genderLocked)?'cls-locked':''}"
      ...
      ${lockVeil}
      <div class="class-icon" style="display:flex;align-items:center;justify-content:center;height:62px">${CLASS_SVG[cls.id]||'<span style="font-size:36px">'+cls.icon+'</span>'}</div>
